...
Data is collected throughout the stop and some variables like `search_base_incident_to_arrest` are observed explicitly **after** an arrest has occurred, whilst others like `suspect_height` are unchanged from before the arrest. Some other variables like `search_flag` are a measure of an activity that may have occurred either before or after a potential arrest. We drop any variables which clearly take place after an arrest, our outcome of interest, since they cannot be used for our prediction question. We maintain unclear variables but address this later.

We drop spatial features other than x and y coordinates of the stop, as this is the most granular spatial information and we use this to map each stop to census tracts to obtain demographic information corresponding to the location of the stop. These dropped spatial features also have high cardinality, which would add many dummies to our model, undermining computational efficiency.
...
The CDF shows us that the youngest and oldest men are more likely to be arrested when stopped, especially when compared with women. This suggests that young adult men are “overstopped” relative to how much they should be.
...
We also wanted to consider how stops and arrests were distributed across NYC.
...
It is difficult to make any clear discernations due to the high frequency of stops, however, most stops and arrests appear to occur in a corridor along the Bronx, Manhattan and Brooklyn. Particularly around the Broknx and Central Brooklyn. This would match up with our earlier suggestion that that these areas have a lower arrest rate because they are more heavily policed.
...
To get proper analysis of what the most important predictors of arrest are we run a series estimation methods. We do a logit regression, shrinkage estimation and a random forest estimation. To avoid overfitting issues, we split into training, test, and when appropriate validation samples.
