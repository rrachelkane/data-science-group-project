...
X <- model.matrix(~ . - suspect_arrested_flag - stop_id, data = sqf_data)
...
```{r}
# insert most important coef here
```
...
# # split into training and validation sets
 X_train_final <- X_train[id_split, ]
 y_train_final <- y_train[id_split]
 X_val <- X_train[-id_split, ]
...
# cat("dimensions of X_val:", dim(X_val), "\n")
...
# train_acc <- test_acc <- oob_acc <- numeric(ncol(X_train))
# 
# # tune random forest for max_features
# for (max_features in 1:15) { # setting this < ncol(X_train_final for now, just to check if it runs)
#   
#     # fit the rf model using training data
#     rf <- randomForest(x = X_train_final, y = factor(y_train_final), mtry = max_features, ntree = ntree)
# 
...
# plot(seq_len(ncol(X_train)), train_acc, type = 'l', main = 'tuning mtry', ylab = 'accuracy', xlab = 'number of features', col = 'blue')
# lines(seq_len(ncol(X_train)), test_acc, col = 'red')
# lines(seq_len(ncol(X_train)), oob_acc, lty = 2, col = 'green')
# legend('bottomright', legend = c('train accuracy', 'validation accuracy', 'oob accuracy'), col = c('blue', 'red', 'green'), lty = c(1, 1, 2), bty = 'n')
# 
# # train final random forest model with optimal m
# RFTuned <- randomForest(x = X_train_final, y = factor(y_train_final), mtry = m_star, ntree = ntree)
# 
...
# abline(a = 0, b = 1, lty = 2, col = 'gray')
# legend('bottomright', legend = c(sprintf("rf auc: %.3f", RF_auc), 'random guess'), lty = c(1, 2), lwd = 2, col = c('darkblue', 'gray'))
# 
# # generate confusion matrix
# cm_rf <- table(true = y_test, predicted = RFPred)
# cm_rf_df <- as.data.frame(cm_rf) %>%
#   group_by(true) %>%
#   mutate(relative_frequency = Freq / sum(Freq))
# 
# # print confusion matrix
# print(cm_rf)
# 
# # plot confusion matrix
# ggplot(cm_rf_df, aes(x = as.factor(true), y = as.factor(predicted), fill = relative_frequency)) +
#   geom_tile() +
#   geom_text(aes(label = sprintf("%.2f", relative_frequency)), color = "white") +
#   scale_fill_gradient(low = "blue", high = "red", name = "rel. freq.") +
#   labs(title = "confusion matrix for random forest", x = "true", y = "predicted") +
#   theme_minimal()
...
# 
# # plot rf var importance - top 20
# ggplot(rf_importance %>% arrange(desc(Scaled_Importance)) %>% slice(1:20), 
#        aes(x = reorder(Variable, Scaled_Importance), y = Scaled_Importance)) +
#   geom_bar(stat = "identity", fill = "blue", alpha = 0.7) +
#   coord_flip() +
#   labs(title = "Top 20 Random Forest Variable Importance",
#        x = "Variable",
#        y = "Scaled Importance") +
#   theme_minimal()
...
## Random Forest for Subset

```{r}
# Subset training data for validation split
n_train_subset <- nrow(X_train_subset) # Use subset for training
id_split_subset <- sample(1:n_train_subset, floor(0.5 * n_train_subset)) # 50% split for training and validation

# Split into training and validation sets for X only
X_train_subset_final <- X_train_subset[id_split_subset, ]
X_val_subset <- X_train_subset[-id_split_subset, ]

# Print dimensions
cat("Dimensions of X_train_subset_final:", dim(X_train_subset_final), "\n")
cat("Dimensions of X_val_subset:", dim(X_val_subset), "\n")

# Random forest tuning parameters
ntree <- 500 # R default

# Default bootstrap sample size is nrow(X_train_subset_final)

# Initialize performance storage
train_acc_subset <- test_acc_subset <- oob_acc_subset <- numeric(ncol(X_train_subset_final))

# Tune random forest for max_features
for (max_features in 1:15) { # Ensure < ncol(X_train_subset_final) for testing
  
    # Fit the rf model using training data
    rf <- randomForest(x = X_train_subset_final, y = factor(y_train_final), 
                       mtry = max_features, ntree = ntree)

    # Use trained model to predict y with validation x
    y_pred_val_subset <- predict(rf, X_val_subset)

    # Use trained model to predict y with training x
    y_pred_train_subset <- rf$predicted
    
    # Compute OOB accuracy measure - OOB error rate
    oob_acc_subset[max_features] <- 1 - rf$err.rate[ntree, "OOB"]

    # Compute training accuracy
    train_acc_subset[max_features] <- mean(y_train_final == 
                                            as.numeric(levels(y_pred_train_subset)[y_pred_train_subset]))
    
    # Compute test (val) accuracy - key parameter of interest
    test_acc_subset[max_features] <- mean(y_val == 
                                           as.numeric(levels(y_pred_val_subset)[y_pred_val_subset]))
}

# Identify best max_features in validation data
m_star_subset <- which.max(test_acc_subset)
cat("Optimal mtry (max_features):", m_star_subset, "\n")

# Train final random forest model with optimal mtry
RFTuned_subset <- randomForest(x = X_train_subset_final, y = factor(y_train_final), 
                               mtry = m_star_subset, ntree = ntree)

# Extract Random Forest importance
rf_importance <- data.frame(
  Variable = rownames(RFTuned_subset$importance),
  Scaled_Importance = RFTuned_subset$importance[, "MeanDecreaseGini"] / max(RFTuned_subset$importance[, "MeanDecreaseGini"], na.rm = TRUE)
)
rf_importance <- rf_importance %>%
  arrange(desc(Scaled_Importance))
...
# Predict using tuned model on test set
RFPred_subset <- predict(RFTuned_subset, newdata = X_test_subset)

# Compute fitted probabilities with tuned model on test set
RFProb_subset <- predict(RFTuned_subset, newdata = X_test_subset, type = "prob")

# Compute ROC and AUC for Random Forest
RF_roc_subset <- roc(response = factor(y_test), predictor = RFProb_subset[, 2], levels = c(0, 1))
RF_auc_subset <- round(auc(RF_roc_subset), 4)
cat("Random Forest AUC (Subset):", RF_auc_subset, "\n")

# Plot ROC curve
plot(RF_roc_subset, main = 'ROC Curve for Random Forest (Subset)', col = 'darkblue', lwd = 2)
abline(a = 0, b = 1, lty = 2, col = 'gray')
legend('bottomright', legend = c(sprintf("RF AUC: %.3f", RF_auc_subset), 'Random Guess'), lty = c(1, 2), lwd = 2, col = c('darkblue', 'gray'))

# Generate confusion matrix
cm_rf_subset <- table(true = y_test, predicted = RFPred_subset)
cm_rf_subset_df <- as.data.frame(cm_rf_subset) %>%
  group_by(true) %>%
  mutate(relative_frequency = Freq / sum(Freq))
...
# Plot confusion matrix
ggplot(cm_rf_subset_df, aes(x = as.factor(true), y = as.factor(predicted), fill = relative_frequency)) +
  geom_tile() +
  geom_text(aes(label = sprintf("%.2f", relative_frequency)), color = "white") +
  scale_fill_gradient(low = "blue", high = "red", name = "Rel. Freq.") +
  labs(title = "Confusion Matrix for Random Forest (Subset)", x = "True", y = "Predicted") +
  theme_minimal()
