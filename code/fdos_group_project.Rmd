...
# drop outlier observations where height is above 300 lbs. and below 90 lbs.
...
# borough of stop & arrests

# time of day of stop & arrests

# issuing officer rank & arrests
```
...
We split our sample as [justify].

For each model type, we estimate using two different matrices of predictors to test sensitivity of prediction accuracy:

* the first drops `stop_id`, due to its irrelevance for prediction, and also  `search_basis_incidental_to_arrest_flag`, as this indicates a search was carried out **after** an arrest. As we are ultimately interested in predicting arrest status ex-ante to the stop from the police officer's perspective [edit]

* the second matrix - subset - also drops all further variables related to search, frisk and physical force. We drop these for a conservative approach to prediction, as it is unclear when these occur in relation to the arrest [edit].
...
X <- model.matrix(~ . - suspect_arrested_flag - stop_id -search_basis_incidental_to_arrest_flag, data = sqf_data) #remove unique id
...
Now the data is prepared, we want to specify the data we will use for our logit model. Our logit model will serve as a simple baseline model. 

* To create the simple model we will drop variables that may have occurred after the stop was initiated (such as `frisked_flag`, `firearm_flag`, and `suspect_eye_color`), as it is unclear when these occurred in relation to the arrest.

* We also drop variables that may lead to multicollinearity (`suspect_hair_color` as that may be closely related to race and `supervising_officer_rank` as that may be closely related to `issuing_officer_rank`). 

* We also dropped `year2` because it is 2023 for all observations. 

```{r}
# Set X_logit, removing unwanted columns based on the patterns
X_logit <- X[, !grepl("^(suspected_crime_description|frisked_flag|firearm_flag|knife_cutter_flag|other_weapon_flag|weapon_found_flag|other_contraband_flag|physical_force|search_basis|suspect_eye_color|suspect_hair_color|supervising_officer_rank)", colnames(X)) & !colnames(X) %in% c("year2")]
```
...
predict_logit <- predict(logit, newdata = X_test_logit_df, type = "response")

roc_sub <- roc(y_test, predict_logit)  
auc_sub <- round(auc(roc_sub), 4)  
cat("AUC for the logit model", auc_sub, "\n")

```
To see how our model performs, we will plot the ROC Curve and the confusion matrix for the logistic regression model. 
```{r}
# Compute ROC and AUC for the model with all features
roc_logit <- roc(y_test, predict_logit)
auc_logit <- round(auc(roc_logit), 4)
cat("AUC for the logit model:", auc_logit, "\n")

# Function to compute and plot combined ROC curves
  # Plot the full model ROC curve
  plot(y = roc_logit$sensitivities, x = 1 - roc_logit$specificities, type = 'l',
  ...
  grid()
  legend('bottomright',
       legend = c(sprintf("AUC: %.3f", auc_logit)),
       lty = c(1, 1), lwd = 2, col = c('blue', 'gray'))
...
predict_logit_binary <- ifelse(predict_logit > 0.5, 1, 0)

# plot confusion matrix for logistic regression with all variables
generate_cm(y_test, predict_logit_binary, "Confusion Matrix for Logistic Regression")

#Calculate mean squared error for both models
mse_logit <- mean((predict_logit - y_test)^2)

print(mse_logit)

```

We now run penalised regressions; LASSO, Ridge and Elastic Net.
...
-Note Lucas: Here is the updated elastic net regression which optimises between ridge and lasso.

##Cross-Validation Set Generations for Linear Shrinkage

```{r}
#Create 3-way data partition for a two-way parametre optimisation in linear shrinkage estimator
set.seed(2)

#Split test data away from training and validation data
train_val_en_index <- createDataPartition(y, p =0.8, list=FALSE)
y_test_en <- y[-train_val_en_index]
X_test_en <- X[-train_val_en_index, ]
X_test_en_subset <- X_subset[-train_val_en_index, ]

#Split training and validation data
train_en_index <- createDataPartition(y[train_val_en_index], p=0.5, list=FALSE)
y_train_en <- y[train_en_index]
X_train_en <- X[train_en_index, ]
X_train_en_subset <- X_subset[train_en_index, ]

y_val_en <- y[-train_en_index]
X_val_en <- X[-train_en_index, ]
X_val_en_subset <- X_subset[-train_en_index, ]
```

## Shrinkage Estimator Optimisation

```{r}
#Optimise elastic net over range of alphas

alpha_values <- seq(0, 1, by = 0.01)  # Range of alpha to test
# Initialize with the lowest possible values
...
for (alpha in alpha_values) {
  ...
  en_opt <- cv.glmnet(X_train_en, y_train_en, alpha = alpha, family = "binomial", type.measure = "class")
  
  #Get the best lambda from cross-validation
  en_opt_lambda_min <- en_opt$lambda.min
  
  # Predict probabilities on the validation set - need to update with X_validate
  en_opt_y_prob <- predict(en_opt, s = en_opt_lambda_min, newx = X_val_en, type = "response")
  
  # Compute AUC
  en_opt_auc_value <- auc(response = y_val_en, predictor = en_opt_y_prob)
  
  # Update best alpha if AUC improves
  if (en_opt_auc_value > en_opt_best_auc) {
...
en_opt_y_test <- predict(en_opt, s = en_opt_lambda_min, newx = X_test_en, type = "response")
en_opt_best_auc <- auc(response = y_test_en, predictor = en_opt_y_test)

# Output best alpha, lambda and its AUC
cat(sprintf("Best Lambda: %.5f\n", en_opt_best_lambda))
cat(sprintf("Best Alpha: %.2f\n", en_opt_best_alpha))
cat(sprintf("Highest AUC: %.5f\n", en_opt_best_auc))

# Best alpha is stored in `en_opt_best_lambda` with lambda = `en_opt_best_lambda`
en_opt <- cv.glmnet(X_train_en, y_train_en, alpha=en_opt_best_alpha, family="binomial", type.measure = "class")
...
# plot coefficients
plot(en_opt$glmnet.fit, xvar = "lambda", label = TRUE)
...
en_opt_predict <- predict(en_opt, s = en_opt_best_lambda, X_test_en, type = "response")
nrow(en_opt_predict)
...
generate_cm(y_test_en, en_opt_y_hat, "Confusion Matrix for Elastic Net (Full Model)")

# compute roc object for subset ridge model
en_opt_roc_full <- roc(response = y_test_en, predictor = en_opt_predict)
```

## Linear Shrinkage Sensitivity Analysis - Over Subset

```{r}

#Optimise elastic net over range of alphas

# Recall alpha_values <- seq(0, 1, by = 0.01) 

# Initialize with the lowest possible values
en_opt_best_alpha_subset <- NULL
en_opt_best_auc_subset <- 0
en_opt_best_lambda_subset <- NULL

# Iterate over alpha values
for (alpha in alpha_values) {
  #Train Elastic Net model for the given alpha
  en_opt_subset <- cv.glmnet(X_train_en_subset, y_train_en_subset, alpha = alpha, family = "binomial", type.measure = "class")
  
  #Get the best lambda from cross-validation
  en_opt_lambda_min_subset <- en_opt_subset$lambda.min
  
  # Predict probabilities on the validation set - need to update with X_validate
  en_opt_y_prob_subset <- predict(en_opt_subset, s = en_opt_lambda_min_subset, newx = X_val_en_subset, type = "response")
  
  # Compute AUC
  en_opt_auc_value_subset <- auc(response = y_val_en_subset, predictor = en_opt_y_prob_subset)
  
  # Update best alpha if AUC improves
  if (en_opt_auc_value_subset > en_opt_best_auc_subset) {
    en_opt_best_auc_subset <- en_opt_auc_value_subset
    en_opt_best_alpha_subset <- alpha
    en_opt_best_lambda_subset <- en_opt_lambda_min
  }
}

en_opt_y_test_subset <- predict(en_opt_subset, s = en_opt_lambda_min_subset, newx = X_test_en_subset, type = "response")
en_opt_best_auc_subset <- auc(response = y_test_en_subset, predictor = en_opt_y_test_subset)

# Output best alpha, lambda and its AUC
cat(sprintf("Best Lambda: %.5f\n", en_opt_best_lambda_subset))
cat(sprintf("Best Alpha: %.2f\n", en_opt_best_alpha_subset))
cat(sprintf("Highest AUC: %.5f\n", en_opt_best_auc_subset))

# Best alpha is stored in `en_opt_best_lambda_subset` with lambda in `en_opt_best_lambda_subset`

 en_opt_subset <- cv.glmnet(X_train_en_subset, y_train_en_subset, alpha = en_opt_best_alpha_subset, family = "binomial", type.measure = "class")
...
plot(en_opt_subset$glmnet.fit, xvar = "lambda", label = TRUE)
...
en_opt_predict_subset <- predict(en_opt_subset, s = en_opt_best_lambda_subset, X_test_subset_en, type = "response")

# add variable for predicted classes for subset
en_opt_y_hat_subset <- ifelse(en_opt_predict_subset > 0.5, 1, 0)

# plot confusion matrix for en_subset regression out of sample
generate_cm(y_test_en, en_opt_y_hat_subset, "Confusion Matrix for Elastic Net (Subset Model)")

# compute roc object for subset ridge model
en_opt_roc_subset <- roc(response = y_test_en, predictor = en_predict_subset)

# plot combined roc curve for ridge models
plot_combined_roc(en_opt_roc_full, en_opt_roc_subset, "Elastic Net Full", "Elastic Net Subset")
```
#Here on out is redundant for shrinkage estimation
## LASSO
...
Now, we tune $\alpha$, as [demonstrate rationale]

NB here - I just use the subset in random forest, because 1 it takes a while to run, so not gonna do both, and 2 its more conservative for ex-ante arrest prediction - cops are interested in predicting arrest before theyve began to interact with the person, just observables available to them before working the stop

i think here we should tune alpha then for the subset example
...
```
## End of irrelevant part

## Variable Importance

top 20 for logit, tuned shrinkage
...
# Plot accuracy metrics for tuning mtry (max_features) at the optimal ntree
plot(results$mtry[results$ntree == best_params$ntree], results$train_acc[results$ntree == best_params$ntree],
     type = 'l', main = 'Tuning mtry for Optimal ntree',
     ylab = 'Accuracy', xlab = 'Number of Features (mtry)', col = 'blue')
lines(results$mtry[results$ntree == best_params$ntree], results$test_acc[results$ntree == best_params$ntree], col = 'red')
lines(results$mtry[results$ntree == best_params$ntree], results$oob_acc[results$ntree == best_params$ntree], lty = 2, col = 'green')
