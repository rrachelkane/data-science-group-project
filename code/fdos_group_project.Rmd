...
# officer not explained stop description? any others that we definitely dont want to use for prediction
# check new dim
dim(sqf_data)
```
...
We split our sample as

We implement cross-validation in our training sample as ... . We use the default $k=10$ folds for computational reasons.
...
**note here on what misclassification error is**

**note here the options for binomial are deviance (default), auc, and class - maybe we do class and then auc and look at differences?**

## LASSO 

**insert formula**

note here - add training accuracy measures, type measure AUC? to compare

```{r}
# set training and test samples
y <- sqf_data$suspect_arrested_flag

# NB NB note here - still some stuff to be dropped here for regressors  (non-normalized age variables, after changing around ordering of summary stats above, + searched/frisk? especially if search is incidental to arrest!?)

# as not converging with ~570 regressors, lets test performance without some here

X <- model.matrix(~ . - suspect_arrested_flag - stop_id - supervising_officer_command_code - searched_flag, data = sqf_data)


# split data
train_index <- createDataPartition(y, p = 0.7, list = FALSE)

# gen y and x train and test
...
commenting out now for runtime
