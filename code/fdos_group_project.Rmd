...
Before addressing outliers, we first need to transform the height variable as it is currently in the format of feet.inches. For example, an observation of 5.10 means 5 feet 10 inches and 5.1 means 5 feet 1 inch. We will transform the inches into feet so each observation is only in feet.
Bear in mind, in the raw data we have values such as 5.90, which really means 5 feet 9 inches, not 5 feet 90 inches. To address this during the conversion, we checked to see if the inches amount had 1 or 2 decimals and handled accordingly. We also ensured all the inches amounts were in between 0 and 11.5 to ensure none were accidentally converted incorrectly. 
```{r}
...
convert_to_feet <- function(feet_inches) {
  ...
  if (inches < 0 || inches > 11.5) {
    warning(paste("Invalid height input:", feet_inches, "- Inches must be between 0 and 11.5. Returning NA."))
