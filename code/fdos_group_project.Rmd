...
```{r}
# 1/ keep tract and acs predictors as needed here in sqf_data by merging those cols by stop_id

# 2/ drop the then in sqf_data which have missing values in any of these predictors
```
...
```{r}
# NB!!!! use a version of data which drops irrelevant spatial attributes made in sf, 

# just keep nta names and ACS data we want for prediction by merging those columns from the sf object into the original.
