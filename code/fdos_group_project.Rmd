...
p_load(readxl, dplyr, ggplot2, knitr, lubridate, tidyr, sf, httr, caret, glmnet, stringr, remotes, RColorBrewer, viridis, scales, classInt, forcats)
```
...
# we will bin stop_hour to see what time of day the stop occurred 

# Create a new variable 'time_of_day' based on 'stop_hour'
sqf_data <- sqf_data %>%
  mutate(
    time_of_day = case_when(
      stop_hour %in% c("05", "06", "07", "08", "09", "10", "11") ~ "Morning",
      stop_hour %in% c("12", "13", "14", "15", "16", "17") ~ "Afternoon",
      stop_hour %in% c("18", "19", "20", "21") ~ "Evening",
      stop_hour %in% c("22", "23", "00", "01", "02", "03", "04") ~ "Night",
...
variables_to_summarize <- c("observed_duration_minutes", "stop_duration_minutes", "suspect_reported_age", "suspect_weight", "suspect_height")
...
variables_to_normalize <- c("observed_duration_minutes", "stop_duration_minutes", 
...
# List of variables to summarize
variables_to_summarize <- c("norm_observed_duration_minutes", "norm_stop_duration_minutes", "norm_suspect_reported_age", "norm_suspect_weight", "norm_suspect_height")
...
# NB - others X/predictors of interest here  
# grouped tables, bars
