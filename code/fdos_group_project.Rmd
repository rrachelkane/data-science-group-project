...
p_load(readxl, dplyr, ggplot2, knitr, lubridate, tidyr, sf, httr, caret, glmnet, stringr, remotes, RColorBrewer, viridis, scales, classInt, forcats, pROC)
```
...
Next, we drop all columns which cannot be used for our prediction question, as they are realized **after** the outcome of interest, namely `arrested_flag`, or are irrelevant for other reasons. We drop spatial features other than x and y coordinates of the stop, as this is the most granular spatial information and we use this to map each stop to census tracts to obtain demographic information corresponding to the location of the stop.

```{r}
sqf_data <- sqf_data %>% 
  select(- c("stop_frisk_date", "record_status_code", "supervising_action_corresponding_activity_log_entry_reviewed", "stop_location_sector_code", "stop_location_apartment", "stop_location_full_address", "stop_location_patrol_boro_name", "stop_location_street_name", "suspect_other_description", "observed_duration_minutes", "stop_duration_minutes", "summons_issued_flag", "supervising_officer_command_code"))
...
sqf_data <- sqf_data %>%
  mutate(
    time_of_day = case_when(
      str_extract(stop_frisk_time, "^\\d{2}") %in% c("05", "06", "07", "08", "09", "10", "11") ~ "Morning",
      str_extract(stop_frisk_time, "^\\d{2}") %in% c("12", "13", "14", "15", "16", "17") ~ "Afternoon",
      str_extract(stop_frisk_time, "^\\d{2}") %in% c("18", "19", "20", "21") ~ "Evening",
      str_extract(stop_frisk_time, "^\\d{2}") %in% c("22", "23", "00", "01", "02", "03", "04") ~ "Night",
...
# height by arrest flag
ggplot(sqf_data, aes(x = suspect_height, color = factor(suspect_arrested_flag))) +
  stat_ecdf(geom = "step") +
...
# NB - others X/predictors of interest here - add some cop-side stuff
# grouped tables, bars

# NB - scatters, corr matrices
# check for any outliers visually  and transform as needed
...
First we will look at our six numerical variables: "stop_id", "year2"  "observed_duration_minutes", "stop_duration_minutes", "suspect_reported_age", "suspect_weight", "suspect_height"

Since "stop_id" is the number given to each unique incident and "year2" is the year the stop occured which is 2023 for all observations so we can ignore these two numerical variables. 

Let's take a look at summary statistics for the other numerical variables.

**note** - maybe instead of doing this here, we do it after the current plots that use age and these variables, then show this after and clean by mutating rather than creating a new variable and dropping the old? not sure
...
We implement cross-validation in our training sample as [edit] . We use the default $k=10$ folds for computational reasons.
...
# set entire matrix of predictors
X <- model.matrix(~ . - suspect_arrested_flag, data = sqf_data)

# set the subset of predictors
...
# gen X train and test, and same for subsets
...
# add variable for classes
lasso_y_hat <- ifelse(lasso_predict > 0.5, 1, 0)

# define function to generate data for confusion matrix
generate_cm <- function(true, predicted) {
  as.data.frame(table(True = true, Predicted = predicted)) %>%
    group_by(Predicted) %>%
    mutate(Predicted_pct = Freq / sum(Freq))
}

# calculate confusion matrix for lasso regression out of sample
lasso_cm <- generate_cm(y_test, lasso_y_hat)

# print table
print(lasso_cm)

#plot
ggplot(data = lasso_cm, mapping = aes(x = ordered(True, c(1, 0)), y = Predicted, fill = Predicted_pct)) +
  geom_tile() +
  geom_text(aes(label = round(Predicted_pct, 2)), color = 'white') +
  scale_fill_gradient(low = "blue", high = "red", name = "Rel. Freq.") +
  xlab("True") +
  ylab("Predicted") +
  labs(title = "Confusion Matrix for LASSO Regression (Test Data)") +
  theme_minimal()

# define function for ROC
compute_roc_auc <- function(y_true, y_pred, title) {
  # Ensure y_pred is a numeric vector, not a matrix
  y_pred <- as.vector(y_pred)

  # Cbind true and predicted values into a single data frame
  combined_df <- cbind.data.frame(True = y_true, Predicted = y_pred)

  # Compute ROC and AUC using the combined data frame
  roc_obj <- roc(response = combined_df$True, predictor = combined_df$Predicted, levels = rev(levels(factor(combined_df$True))))
  auc_value <- round(auc(roc_obj), 4)

  # Generate the ROC plot using ggplot2
  roc_plot <- ggroc(roc_obj, legacy.axes = TRUE, color = "blue", size = 1) +
    ggtitle(paste0(title, " (AUC = ", auc_value, ")")) +
...
lasso_roc <- compute_roc_auc(y_test, lasso_predict, "ROC Curve for LASSO Out of Sample")
```
...
# calculate confusion matrix for lasso regression out of sample
lasso_cm_subset <- generate_cm(y_test, lasso_y_hat_subset)

# print table
print(lasso_cm_subset)

#plot
ggplot(data = lasso_cm_subset, mapping = aes(x = ordered(True, c(1, 0)), y = Predicted, fill = Predicted_pct)) +
  geom_tile() +
  geom_text(aes(label = round(Predicted_pct, 2)), color = 'white') +
  scale_fill_gradient(low = "blue", high = "red", name = "Rel. Freq.") +
  xlab("True") +
  ylab("Predicted") +
  labs(title = "Confusion Matrix for LASSO Regression (Test Data - Subset)") +
  theme_minimal()

lasso_roc_subset <- compute_roc_auc(y_test, lasso_predict_subset, "ROC Curve for LASSO Out of Sample (Subset")
```
...
# calculate confusion matrix for ridge regression out of sample
ridge_cm <- generate_cm(y_test, ridge_y_hat)

# print table
print(ridge_cm)

# plot confusion matrix
ggplot(data = ridge_cm, mapping = aes(x = ordered(True, c(1, 0)), y = Predicted, fill = Predicted_pct)) +
  geom_tile() +
  geom_text(aes(label = round(Predicted_pct, 2)), color = 'white') +
  scale_fill_gradient(low = "blue", high = "red", name = "rel. freq.") +
  xlab("true") +
  ylab("predicted") +
  labs(title = "confusion matrix for ridge regression (test data)") +
  theme_minimal()

# compute and plot roc for ridge
ridge_roc <- compute_roc_auc(y_test, ridge_predict, "roc curve for ridge out of sample")
```
...
# calculate confusion matrix for ridge regression (subset)
ridge_cm_subset <- generate_cm(y_test, ridge_y_hat_subset)

# print table
print(ridge_cm_subset)

# plot confusion matrix
ggplot(data = ridge_cm_subset, mapping = aes(x = ordered(True, c(1, 0)), y = Predicted, fill = Predicted_pct)) +
  geom_tile() +
  geom_text(aes(label = round(Predicted_pct, 2)), color = 'white') +
  scale_fill_gradient(low = "blue", high = "red", name = "Rel. Freq.") +
  xlab("True") +
  ylab("Predicted") +
  labs(title = "Confusion Matrix for Ridge Regression (Test Data - Subset)") +
  theme_minimal()

# compute and plot roc for ridge (subset)
ridge_roc_subset <- compute_roc_auc(y_test, ridge_predict_subset, "ROC Curve for Ridge Out of Sample (Subset)")
...
# calculate confusion matrix for elastic net regression out of sample
en_cm <- generate_cm(y_test, en_y_hat)

# print table
print(en_cm)

# plot confusion matrix
ggplot(data = en_cm, mapping = aes(x = ordered(True, c(1, 0)), y = Predicted, fill = Predicted_pct)) +
  geom_tile() +
  geom_text(aes(label = round(Predicted_pct, 2)), color = 'white') +
  scale_fill_gradient(low = "blue", high = "red", name = "rel. freq.") +
  xlab("true") +
  ylab("predicted") +
  labs(title = "confusion matrix for elastic net regression (test data)") +
  theme_minimal()

# compute and plot roc for elastic net
en_roc <- compute_roc_auc(y_test, en_predict, "roc curve for elastic net out of sample")
```
...
# add variable for classes for subset
en_y_hat_subset <- ifelse(en_predict_subset > 0.5, 1, 0)

# calculate confusion matrix for elastic net subset regression out of sample
en_cm_subset <- generate_cm(y_test, en_y_hat_subset)

# print table for subset
print(en_cm_subset)

# plot confusion matrix for elastic net subset
ggplot(data = en_cm_subset, mapping = aes(x = ordered(True, c(1, 0)), y = Predicted, fill = Predicted_pct)) +
  geom_tile() +
  geom_text(aes(label = round(Predicted_pct, 2)), color = 'white') +
  scale_fill_gradient(low = "blue", high = "red", name = "Rel. Freq.") +
  xlab("True") +
  ylab("Predicted") +
  labs(title = "Confusion Matrix for Elastic Net Regression (Test Data - Subset)") +
  theme_minimal()

# compute and plot roc for elastic net subset
en_roc_subset <- compute_roc_auc(y_test, en_predict_subset, "ROC Curve for Elastic Net Out of Sample (Subset)")
...
# Lucas code
Notes (I couldn't get the MSE calculations to work, so they need some adjusting. The code was taking forever to run so I think each part works ok (apart from the MSE) but not certain about how it runs together. Finally, I am not sure I have got the cross-validation exactly right for elastic_net)

**Note** - just commenting this out so it runs fine, until can check this
...
# Y <- sqf_data$suspect_arrested_flag
# 
# # Use model.matrix to encode factors as dummy variables and remove stop_id
# # Keep all variables including dummies with little variation since
# # shrinkage estimation will shrink any estimators with no predictive power
# # down to or close to 0.
# X <- model.matrix(~ . - stop_id - suspect_arrested_flag, data = sqf_data)[, -1]
# 
# # Split the data into training and testing sets
# set.seed(1)  # Set seed for reproducibility
# 
# train_indices <- createDataPartition(Y, p = 2/3, list = FALSE)
# 
...
# 
# lasso_predict <- predict(lasso_model, s = best_lambda_lasso, X_test, type = "response")
# mse_lasso <- mean((Y_test-cbind(1, X_test) %*% coef(lasso_predict, s='best_lambda_lasso'))**2)
# cat("Mean Squared Error on Lasso predictions:", mse_lasso, "\n")
```
...
# 
# ridge_predict <- predict(ridge_model, s = best_lambda_ridge, newx = X_test, type = "response")
# mse_ridge <- mean((Y_test-cbind(1, X_test) %*% coef(ridge_predict, s='best_lambda_ridge'))**2)
# cat("Mean Squared Error on ridge predictions:", mse_ridge, "\n")
...
# set.seed(4)
# 
# # Split training data into new training and validation data
# 
# validate_indices <- createDataPartition(Y_train, p = 1/2, list = FALSE)
# 
# X_train_en <- X_train[!validate_indices, ]
# Y_train_en <- Y_train[!validate_indices]
# 
# X_validate_en <- X_train[validate_indices, ]
# Y_validate_en <- Y_train[validate_indices]
# 
# X_test_en <- X_test
# Y_test_en <- Y_test
# 
# elastic_net_model <- cv.glmnet(X_train, Y_train, alpha = 0, family="binomial")
# 
# best_alpha_en <- NULL
# best_model_en <- NULL
# best_lambda_en <- NULL
# lowest_validation_mse <- Inf
# 
# alpha_values <- seq(0, 1, by = 0.01)
# 
# for (alpha in alpha_values) {
#   #Elastic Net model for varying alpha
#   en_model <- cv.glmnet(X_train_en, Y_train_en, alpha = alpha, family = "binomial"
#   )
#   
# en_validate <- predict(en_model, s = en_model$lambda.min, X_validate_en, type = "response")
# 
# mse_validate_en <- mean((Y_validate_en-cbind(1, X_validate_en) %*% coef(en_validate,s='best_lamda_en'))**2)
#   
#   # Update best alpha and model if this alpha gives lower MSE
#   if (mse_validate_en < lowest_validation_mse) {
#     lowest_validation_mse <- mse_validate_en
#     best_alpha_en <- alpha
#     best_model_en <- en_model
#     best_lambda_en <- en_model$lambda.min
#   }
# }
# 
# cat("Best Alpha:", best_alpha_en, "\n")
# cat("Best Lambda (optimized on validation data):", best_lambda_en, "\n")
# 
# mse_test_en <- mean(((Y_test_en-cbind(1, X_test_en) %*% coef(en_validate,s='best_lamda_en'))**2)))
# cat("Mean Squared Error on elastic net predictions:", mse_test_en, "\n")
