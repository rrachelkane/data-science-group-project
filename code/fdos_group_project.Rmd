...
if (is.null(ridge_subset$lambda.min)) {
  stop("Optimal lambda not found for Ridge Regression.")
}
...
Now we implement elastic net regression, choosing to tune both $\lambda$ and $\alpha$ [insert justification].
...
for (alpha in alpha_values) {
  ...
  en_opt <- cv.glmnet(X_train_subset_final, y_train_final, alpha = alpha, family = "binomial", type.measure = "class")
  
  # Get the best lambda from cross-validation
  en_opt_lambda_min <- en_opt$lambda.min
  
  # Predict probabilities on the validation set
  en_opt_y_prob <- predict(en_opt, s = en_opt_lambda_min, newx = X_val_subset, type = "response")
  
  # Compute AUC
  en_opt_auc_value <- auc(response = y_val, predictor = en_opt_y_prob)
...
# Train the Elastic Net model with the best alpha
en_opt <- cv.glmnet(X_train_subset_final, y_train_final, alpha = en_opt_best_alpha, family = "binomial", type.measure = "class")
...
en_opt_predict <- predict(en_opt, s = en_opt_best_lambda, newx = X_test_subset, type = "response")

# compute tuned auc
en_opt_best_auc <- auc(response = y_test, predictor = en_opt_predict)
...
# Plot confusion matrix for Elastic Net regression out of sample
generate_cm(y_test, en_opt_y_hat, "Confusion Matrix for Elastic Net (Subset Model)")

# Compute ROC object for subset ridge model
en_opt_roc_subset <- roc(response = y_test, predictor = en_opt_predict)
